...
function writeTempValuesInScoreTable(dice) {
  ...
  const yatzyElement=document.getElementById("yatzy"+playerNumber);
  if(scoreTable[12]===undefined){
    yatzyElement.innerHTML=yatzyScore;
  } else if(yatzyScore>0 && scoreTable[12]) {
    yatzyScore=parseInt(yatzyElement.innerHTML)+100;
    yatzyElement.innerHTML=yatzyScore;
  }
  if(yatzyScore>0 && scoreTable[dice[0]-1]!=undefined && scoreTable[12]!==undefined){
    jokerCard=true;
  }
  if(yatzyScore>0 && scoreTable[dice[0]-1]==undefined && scoreTable[12]!==undefined){
...
function calculateLowerSectionScore(playerScore){
  ...
  if(yatzy>0) {
    ...
    yatzy=parseInt(document.getElementById("yatzy"+playerNumber).innerHTML);
  }
